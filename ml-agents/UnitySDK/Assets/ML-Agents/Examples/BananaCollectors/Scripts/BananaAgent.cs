...
public class BananaAgent : Agent
{
    ...
    private void print(Func<Room> p)
    {
    ...
    public void MoveAgent(float[] act)
    {
        ...
        Vector3 oldRotateDir = rotateDir;
        ...
        if (!frozen)
        {
            ...
            //AddReward(Vector3.Magnitude(rotateDir - oldRotateDir) * -0.0001f);
        }
    ...
    public override void AgentReset()
    {
        ...
        bananas = 0;
        myLaser.transform.localScale = new Vector3(0f, 0f, 0f);
        //GameObject[] arr = FindGameObjectsWithName("Room")
        transform.position = new Vector3(UnityEngine.Random.Range((float)(loc.x - 0.5 * size.x), (float)(loc.x + 0.5 * size.x)),
                                         2f, UnityEngine.Random.Range((float)(loc.z - 0.5 * size.z), (float)(loc.z + 0.5 * size.z)))
    ...
    protected GameObject[] FindGameObjectsWithName(string name)
    {
        int a = FindObjectsOfType<GameObject>().Length;
        GameObject[] arr = new GameObject[a];
        int FluentNumber = 0;
        for (int i = 0; i < a; i++)
        {
            if (FindObjectsOfType<GameObject>()[i].name == name)
            {
                arr[FluentNumber] = FindObjectsOfType<GameObject>()[i];
                FluentNumber++;
            }
        }
